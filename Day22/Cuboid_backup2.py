...
class Cuboid:
	...
	def Collision(self, c2): # Use this to remove. This can be repurposed when adding
		...
		if (self.xs[0] < c2.xs[0]): # and (SELF_WITHIN_C2 Z SPANS):# if self exists strictly left of c2 AND w/in it's z spans
			...
			temp_cuboid_array.append(Cuboid([self.xs[0], c2.xs[0]-1], self.ys, c2.zs))# [c1.xs[0], c2.xs[0]-1], c1.ys, c2.zs # the left hall
		if c2.xs[1] < self.xs[1]:# for c1 right of c2
			temp_cuboid_array.append(Cuboid([c2.xs[1]+1, self.xs[1]], self.ys, c2.zs))# [c2.xs[1]+1, c1.xs[1]], c1.ys, c2.zs # the right hall
		if self.ys[0] < c2.ys[0]:# for c1 in front of c2
			temp_cuboid_array.append(Cuboid(c2.xs, [self.ys[0], c2.ys[0]-1], c2.zs))# c2.xs, [c1.ys[0], c2.ys[0]-1], c2.zs # the little front bit
		if c2.ys[1] < self.ys[1]:# for c1 in back of c2
			temp_cuboid_array.append(Cuboid(c2.xs, [c2.ys[0]+1, self.ys[1]], c2.zs))# c2.xs, [c2.ys[1]+1, c1.ys[1]], c2.zs # the little back bit
