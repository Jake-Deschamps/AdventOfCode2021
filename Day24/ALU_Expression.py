	...
	def Analyze(self, Inst_In):
		...
		for index, inst in enumerate(Inst_In):
			...
			if not type(out) == type(1): # if its an eql where we got two outcomes
				Comparison, StateF, StateT = out# out is then two different states to continue with
				self.ALU.SetState(StateT)
				self.TruthHistory.append(1)
				self.ComparisonHistory.append(Comparison)
		...
		if index == len(Inst_In)-1: # if we analyzed the last instruction here
			...
			if PolyHasSolution(final_expr) or final_expr == 0:
				as_string = str('')
				as_string += str(final_expr) + " with: \n"
				for TH, CH in zip(self.TruthHistory, self.ComparisonHistory):
					as_string += str(TH) + ': ' + str(CH[0]) + ' = ' + str(CH[1]) + '\n'
				self.FinalExpressions.append(as_string)
...
def FloorDivSymplify(Expression):
	new_expression = 0
	for term in Add.make_args(Expression):
		try:
			...
			coeff = term.coeff(var)
			#print(coeff)
			if (not abs(coeff) < 1/9):
				new_expression += term
		except:
			new_expression += int(term)
...
class ALU_Ex:
	...
	def ParseLine(self, Line):
		...
		if words[0] == 'inp':
		...
		elif words[0] == 'div': # simplify after this operation
			var_a = FloorDivSymplify(var_a/var_b)
			setattr(self, words[1], var_a)
			return 0
			...
			if type(var_a) == type(1) and type(var_b) == type(1):
				...
				if EqualityTest(var_a, var_b):
					State_Array = []
					State_Array.append([var_a, var_b])
