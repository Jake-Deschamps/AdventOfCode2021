			...
			for a in self.Amphipods:
				...
				if (not a.IsDone) and (a.MovesLeft > 0):
					...
					for s in self.DetermineAccessibleSpaces(a):
						if s.Stoppable:
							if a.MovesLeft == 1:
								if s.Type == a.Type: # if it is the right hallway
								
									
									test_coords = s.Coords.copy()
									room_good = True
									checking_room = True
									
									while room_good and checking_room:
										test_coords[1] += 1
										if self.SpaceDict[str(test_coords)].IsOccupied:
											if not self.SpaceDict[str(test_coords)].Amphipod.Type == a.Type: # bad!
												room_good = False
											
										elif self.SpaceDict[str(test_coords)].Type == 'W': # good and done!
											checking_room = False
...
class Space:
	def __init__(self, coords, type):
		...
		if self.Coords[1] == 5: # 3 for part 1, 5 for part 2
			self.Bottom = True
		else:
			self.Bottom = False
